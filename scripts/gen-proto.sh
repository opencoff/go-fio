...
# Usage: $0 proto [proto...]
#
# This script must be run from the root of your project and refer to proto-files
# relative to the project-root. It puts the generated protobuf files in 
#   $projroot/internal/pb
#
# Examples:
#
#   $0 proto/*.proto
#
# This builds the right toolchain if needed and generates the VTproto files. The
# generated files are in ./internal/pb.


#set -x

# Change this to point to your go module for this project
ProjectGoMod="module=github.com/opencoff/go-fio"
...
# We do command line processing here because we want to set important global vars
...
_EOF
    ...
    $e $pgen  \
        ...
        --plugin protoc-gen-go-vtproto="$vtgen" \
        --go_out=$outdir \
        --go_opt="$ProjectGoMod" \
        --go-vtproto_out=$outdir \
        --go-vtproto_opt="$ProjectGoMod,features=marshal+unmarshal+size"  \
         $args || die "can't generate protobuf output for $args .."

    #$e $pgen  \
    #    --go_out=$outdir --plugin protoc-gen-go="$gogen" \
    #    --go_opt="paths=source_relative" \
    #    --go-vtproto_out=$outdir --plugin protoc-gen-go-vtproto="$vtgen" \
    #    --go-vtproto_opt="paths=source_relative,features=marshal+unmarshal+size"  \
    #     $args || die "can't generate protobuf output for $f .."
